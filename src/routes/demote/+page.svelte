<script lang="ts">
  ...
  const managers = $derived(
    ((data.managers ?? []) as Employee[]).filter(e => !demoted.has(e.id))
  )
  ...
  $effect(() => {
    if (form && 'success' in form && form.success && form.id) {
      const emp = (data.managers as Employee[]).find(e => e.id === form.id)
      if (emp) {
...
<svelte:head>
  <title>ลดระดับ Manager — Leave Request System</title>
</svelte:head>

<AppLayout {user} title="ลดระดับ Manager เป็น User">
